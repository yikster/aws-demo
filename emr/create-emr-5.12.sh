...
if [[ $# -lt 12 ]] ; then
  ...
  echo "Usages: ./create-emr.sh cluster_name core_instance_type core_instance_nodes master_instance_type subnet_id master_security_group_id slave_security_group_id keypair_name executor_memory driver_memory driver_cores core_volume_size aws-profile"
  exit 1
fi 
...
--release-label emr-5.12.0 --name "${CLUSTER_NAME}" \
...
--bootstrap-actions Path="s3://awsdemo-ap-northeast-2/emr-bootstrap-actions/hive/configure-Hive-LLAP.sh"
`
